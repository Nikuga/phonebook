...
int main(void)
{
    ...
    double opt_sum_a_hash = 0.0, opt_sum_f_hash = 0.0;
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
        ...
        fscanf(fp, "%s %s %lf %lf\n", append, find, &opt_a, &opt_f);
        opt_sum_a_hash += opt_a;
        opt_sum_f_hash += opt_f;
    }
    fprintf(output, "append() %lf %lf %lf\n", orig_sum_a / 100.0, opt_sum_a / 100.0, opt_sum_a_hash / 100.0);
    fprintf(output, "findName() %lf %lf %lf", orig_sum_f / 100.0, opt_sum_f / 100.0, opt_sum_f_hash / 100.0);
    fclose(output);
    fclose(fp);
    return 0;
}
